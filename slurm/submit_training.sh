...
echo "Submitting training pipeline (htc cluster)..."

# Submit job 1 to htc cluster
J1=$(sbatch --cluster=htc --parsable "$SCRIPT_DIR/jobs/01_generate_graphs.sh" | cut -d';' -f1)
echo "  01_generate_graphs: $J1 (htc)"

# Submit job 2 to htc cluster with native dependency
...
# Submit job 3 to htc cluster with native dependency
